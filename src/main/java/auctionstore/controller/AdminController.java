...
public class AdminController {
    ...
    @GetMapping("/items")
    public String getItems(Pageable pageable, Model model) {
        controllerUtils.addPagination(model, adminService.getItems(pageable));
        return Pages.ADMIN_ITEMS;
    }

    @GetMapping("/items/search")
    public String searchItems(SearchRequest request, Pageable pageable, Model model) {
        controllerUtils.addPagination(request, model, adminService.searchItems(request, pageable));
        return Pages.ADMIN_ITEMS;
    }
    ...
    public String getItem(@PathVariable Long itemId, Model model) {
        model.addAttribute("items", adminService.getItemById(itemId));
        return Pages.ADMIN_EDIT_ITEM;
    }

    @PostMapping("/edit/item")
    public String editItem(@Valid ItemRequest item, BindingResult bindingResult, Model model,
                              @RequestParam("file") MultipartFile file, RedirectAttributes attributes) {
        if (controllerUtils.validateInputFields(bindingResult, model, "item", item)) {
            return Pages.ADMIN_EDIT_ITEM;
        }
        return controllerUtils.setAlertFlashMessage(attributes, "/admin/item", adminService.editItem(item, file));
    }

    @GetMapping("/add/item")
    public String addItem() {
            return Pages.ADMIN_ADD_ITEM;
    }

    @PostMapping("/add/item")
    public String addItem(@Valid ItemRequest item, BindingResult bindingResult, Model model,
                             @RequestParam("file") MultipartFile file, RedirectAttributes attributes) {
        if (controllerUtils.validateInputFields(bindingResult, model, "item", item)) {
            return Pages.ADMIN_ADD_ITEM;
        }
        return controllerUtils.setAlertFlashMessage(attributes, "/admin/item", adminService.addItem(item, file));
    }
